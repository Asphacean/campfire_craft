...
result: [pending] Root cause of v0.1.3's `exit 1` now confirmed (2026-08-31) and fixed in `release.yml` (commit `dedc1ea`); the fix has NOT yet run through a live GitHub Actions job (no new tag cut this session — retry budget stays reserved per the ≤3 rule). Separately, the v0.1.3 feed was published to production **manually**, replicating the publish job's exact commands as the operator, to unblock tests 1/2/4 without spending a retry: see the local-publish note below.
