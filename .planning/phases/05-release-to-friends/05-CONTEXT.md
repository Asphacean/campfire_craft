...
### Repository & CI
- New GitHub repository **`Asphacean/campfire_craft`**, **public**, containing the whole `~/rlcraft` tree as-is (secrets already gitignored: `server.env`, `ca/campfire-ca-key.pem`, `auth/`, `pack/`, `launcher-dist/`, keys). Pushed from the Pi over SSH (key `~/.ssh/id_ed25519`, GitHub user Asphacean). A pre-push secret scan (grep for `RCON_PASSWORD=`, `BEGIN .* PRIVATE KEY`, `.key` files) runs before the first push
